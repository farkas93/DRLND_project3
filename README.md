...
Make sure to adjust the file ending on line 53 if you want to run `main_train.py` and you are not using Windows.
A copy of the solution model can also be found in the project folder in the zip archive `solution_weights.zip`. The zip file contains the weights for actor, cricitc, target actor and target critic for both agents. The zip-archive `solution_weights.zip` is a zipped copy of the folder `saved_models/solution3`. The model name of trained model is always encoded within the two saved weight files `modelname_actor.pth` and `modelname_critic.pth`, where the prefix `modelname` is the model name.
**Example**: For running the test script with the solution weights use following command
```python main_train.py solution3```

Please note that for this project there was no work done in the jupyter notebook and it was left as it was per default.
